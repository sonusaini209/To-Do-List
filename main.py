...
def process_agent_command(request: AgentRequest):
    """AI agent endpoint using fallback pattern matching only"""
    global conversation_history

    # Add to conversation history
    conversation_history.append(
        {"role": "user", "message": request.message, "timestamp": datetime.now().isoformat()}
    )
    ...
    conversation_history.append(
        {"role": "assistant", "message": result["response"], "timestamp": datetime.now().isoformat()}
    )
...
def get_conversation_history():
    ...
    return {"conversation": conversation_history[-10:]}
